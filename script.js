document.addEventListener('DOMContentLoaded', () => {
    ...
    if (progressChartElem) {
        const progressCtx = progressChartElem.getContext('2d');
    ...
    const skillChartElem = document.getElementById('skillChart');
    if (skillChartElem) {
        const skillCtx = skillChartElem.getContext('2d');
        new Chart(skillCtx, {
            type: 'radar',
            data: {
    ...
    let tasks = JSON.parse(localStorage.getItem('cybersec_tasks')) || [];

    function saveTasks() {
        localStorage.setItem('cybersec_tasks', JSON.stringify(tasks));
    }
    ...
    function renderTasks() {
        ...
        tasks.forEach((task, index) => {
            ...
            li.innerHTML = `
                ...
                <i class="fas fa-trash delete-task" onclick="deleteTask(${index})"></i>
            `;
